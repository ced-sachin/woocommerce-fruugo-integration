...
function fruggorenderMetaKeysTableOnProfilePage( $productId ) {
	...
	<table class="wp-list-table widefat fixed striped" id="ced_fruugo_metakeys_list">
		<thead>
			<tr>
				<th><?php esc_html_e( 'Meta Field Key', 'ced-fruugo' ); ?></th>
				<th><?php esc_html_e( 'Meta Field Value', 'ced-fruugo' ); ?></th>
			</tr>	
		</thead>
		<tbody>
			<?php
			...
			foreach ( $getPostCustom as $customPostKey => $customPostValue ) {
				...
				echo '<tr>';
					echo '<td>';
					echo '<input type="checkbox" class="ced_fruugo_add_del_meta_keys" name="unique_post[]" value="' . esc_html($customPostKey) . '" id="' . esc_html($customPostKey) . '" ' . esc_html($checked) . '><label for="' . esc_html($customPostKey) . '">' . esc_html($customPostKey) . '</label>';
					echo '</td>';
					echo '<td>';
					echo esc_html($value);
					echo '</td>';
...
function fruugorenderMetaSelectionDropdownOnProfilePage( $categroy_specific_variation = '' ) {
	...
	$selected = '';
	echo '<select id="' . esc_html($selectId) . '" name="' . esc_html($selectId) . '">';
	echo '<option value="null"> -- select -- </option>';
	if ( is_array( $attrOptions ) ) {
		foreach ( $attrOptions as $attrKey => $attrName ) :
			$selected = '';
			if ( strpos( strtolower( $attrName ), strtolower( $categroy_specific_variation ) ) ) {
				$selected = 'selected';
			}
			echo '<option value="' . esc_html($attrKey) . '" ' . esc_html($selected) . '>' . esc_html($attrName) . '</option>';
		endforeach;
	}
	echo '</select>';
...
function fruugorenderMarketPlacesLinksOnTop( $page = '' ) {
	...
	if ( is_array( $marketPlaces ) ) {
		...
		foreach ( $marketPlaces as $marketPlace ) {
			...
			$redirectURL = get_admin_url() . 'admin.php?page=' . esc_html($page) . '&amp;section=' . esc_html($marketPlace);
			echo '<li>';
			echo '<a href="' . esc_attr($redirectURL) . '" class="' . esc_html($class) . '">' . esc_html(strtoupper( $marketPlace )) . '</a>';
...
function fruugoisMarketPlaceConfigurationsValidated( $marketplaceID ) {
	...
	if ( empty( $ced_fruugo_validate_marketplace ) ||  'no' == $ced_fruugo_validate_marketplace ) {
...
function ced_fruugo_available_marketplace( $api_key = '' ) {
	$dir                   = esc_html(CED_FRUUGO_DIRPATH) . 'marketplaces';
	$folders               = scandir( $dir, 1 );
	$availableMarketPlaces = array();
	foreach ( $folders as $folder ) {
		if ( '.' === $folder || '..' === $folder ||  '.DS_Store' ===$folder ) {
			continue;
		}
...
function ced_fruugo_woo_missing_notice() {
	...
	echo '<div class="error"><p>' . sprintf( esc_html( 'Woocommerce fruugo Integration requires WooCommerce to be installed and active. You can download %s here.', 'ced-fruugo' ), '<a href="http://www.woothemes.com/woocommerce/" target="_blank">WooCommerce</a>' ) . '</p></div>';
}
...
function fruugo_profile_validation( $proID, $condition = array() ) {
	if ( is_array( $condition ) ) {
		...
		if ( ! is_null( $default ) && strlen( $default ) && 'null' != $default ) {
			return $default;
		}

		if ( ! is_null( $bymeta ) && strlen( $bymeta ) &&  'null' !=$bymeta) {
...
function fruugogetProfileDetail( $profileId ) {
	...
	$profileDetail = $wpdb->get_row( $wpdb->prepare( "SELECT * FROM {$wpdb->prefix}ced_fruugo_fruugoprofiles WHERE `id`=%s", $profileId ), 'ARRAY_A' );
...
function fruugo_get_marketplace_price( $proId, $marketplace = '' ) {
	...
	if ( ! is_null( $marketplace ) && strlen( $marketplace ) ) {
		...
		if ( $condition && ! is_null( $condition ) && count( $condition ) ) {
			...
			if ( $price && ! is_null( $price ) &&  'null' != $price && strlen( $price ) ) {
...
function fruugo_get_marketplace_qty( $proId, $marketplace = '' ) {
	...
	if ( null != $marketplace ||  '' != $marketplace ) {
		...
		if ( $condition && ! is_null( $condition ) && count( $condition ) ) {
			...
			if ( $qty && ! is_null( $qty ) && 'null' !=$qty && strlen( $qty ) ) {
				return intval( $qty );
			}
		}
